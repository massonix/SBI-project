...
class Window(Frame):
    ...
    def client_exit(self):
        if messagebox.askyesno("Quit", "Are you sure you want to exit this app?"):
            Frame.quit(self)
    ...
    def create_output_frame(self):
        ...
        self.optimize = BooleanVar(value=False)
        self.verbose_var = BooleanVar(value=False)
        self.st_var = BooleanVar(value=False)

        b1 = Checkbutton(output_options_frame, text="Optimize", variable=self.optimize, onvalue=True, offvalue=False)
        b1.pack(side=LEFT)
    ...
    def showOptions(self):
        ...
        self.message.set("Optimize: %s, Verbose: %s, Stoichiometry: %s, output directory: %s, maximum files: %s" % (
            self.optimize.get(), self.verbose_var.get(), self.st_var.get(), self.output_dir, self.max_files.get()))
